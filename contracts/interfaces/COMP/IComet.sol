...
/**
 * @title According to Compound's Comet Main Interface (without Ext)
 * @dev change: convert public to external
 */
interface IComet {
    struct AssetInfo {
        uint8 offset;
    ...
    function getAssetInfoByAddress(address asset) virtual external view returns (AssetInfo memory);
    ...
    function getSupplyRate(uint utilization) virtual external view returns (uint64);
    function getBorrowRate(uint utilization) virtual external view returns (uint64);
    ...
    /**
     * Comet Public Variable function
     */
